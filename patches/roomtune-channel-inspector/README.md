...
| `src/wx/channel_inspector_dialog.h` (신규) | 비모달 UI(채널행 solo/mute 체크박스 + 피크 미터, 헤더온리 wxDialog). |
...
- solo(v1) = 대상 채널을 모니터 L/R로 모노합(×0.707). HI(6)/VI(7) 포함 전 채널 노출.
...
- **실기 GUI 미검증(사용자 필요):** `Tools > Channel inspector...` 열기 → 채널 행/피크미터 표시,
  solo/mute 실시간 반영, 인스펙터 미사용 시 출력 동등성, DCP 언로드→다른 채널수 로드 시 행 재구성.
